...
procedure TCssFile.Clear; // Vybuluje udaje polozky - datovy baffer
begin
  FillChar (oData,SizeOf(TData),#0)
end;

procedure TCssFile.Add; // Prida udaje z datoveho bufferu do textoveho suboru
begin
  oTxtWrap.ClearWrap;
  With oData do begin
...
procedure TCssFile.First; // Skoci na prvy zaznam
begin
  oIndex := 0;
  LoadData;  // Nacita riadok textoveho suboru do datoveho bufferu
end;

procedure TCssFile.Next; // Skoci na nasledujuci zaznam
begin
...
procedure TCssFile.LoadData; // Nacita riadok textoveho suboru do datoveho bufferu
begin
  oTxtCut.SetStr(oFile.Strings[oIndex]);
  With oData do begin
    CrpNum := oTxtCut.GetNum(1);      // Èíslo vlastnej firmy
    WriNum := oTxtCut.GetNum(2);      // Èíslo prevádzkovej jednotky na ktorý bol vystavený doklad (celé èíslo)
    DocNum := oTxtCut.GetText(3);     // Èíslo dokladu predaja (text max 12 znakov)
    ItmNum := oTxtCut.GetNum(4);      // Èíslo riadku dokladu predaja (celé èíslo)
    DocDate := oTxtCut.GetDate(5);    // Dátum uskutoènenia predaja (formát: dd.mm.yyyy)
    DocTime := oTxtCut.GetText(6);    // Èas uskutoènenia predaja (formát: hh:mm)
    SalType := oTxtCut.GetText(7);    // Typ predaja (text 1 znak, možné hodnoty: C - registraèná pokladòa, D - dodací list, F - faktúra)
    GsCode := oTxtCut.GetNum(8);      // Tovarove cislo (PLU) predaneho tovaru
    MgCode := oTxtCut.GetNum(9);      // Tovarová skupina (celé èíslo)
    FgCode := oTxtCut.GetNum(10);     // Finanèná skupina (celé èíslo)
    GsName := oTxtCut.GetText(11);    // Názov tovaru (text max 30 znakov)
    BarCode := oTxtCut.GetText(12);   // Identifikaèný kód tovaru (text max 15 znakov)
    StkCode := oTxtCut.GetText(13);   // Skladový kód tovaru (text max 15 znakov)
    OsdCode := oTxtCut.GetText(14);   // Objednávací kód tovaru (text max 15 znakov)
    GsQnt := oTxtCut.GetReal(15);     // Množstvo predaného tovaru (desatinné èíslo 3 desatinnými miestami)
    MsName := oTxtCut.GetText(16);    // Merná jednotka tovaru
    VatPrc := oTxtCut.GetReal(17);    // Percentuálna sadzba DPH (desatinné èíslo, formát: ##.#)
    CusCrd := oTxtCut.GetText(18);    // Identifikaèný kód zákazníckej karty (text max 20 znakov)
    PaCode := oTxtCut.GetNum(19);     // Interný kód odberate¾a (celé èíslo)
    RegIno := oTxtCut.GetText(20);    // IÈO odberate¾a (text max 10 znakov)
    PaName := oTxtCut.GetText(21);    // Názov odberate¾a (text max 30 znakov)
    ZipCode := oTxtCut.GetText(22);   // PSÈ obce alebo mesta kam tovar bol predaný (text max 6 znakov)
    DlrCode := oTxtCut.GetNum(23);    // Kód obchodného zástupcu (celé èíslo)
    RspCode := oTxtCut.GetText(24);   // Identifikátor odbytového pracovníka alebo pokladníka, ktorý predal položku (text max 8 znakov)
    RspName := oTxtCut.GetText(25);   // Meno a priezvisko odbytového pracovníka alebo pokladníka, ktorý predal položku (text max 30 znakov)
    AcCValue := oTxtCut.GetReal(26);  // Hodnota predaja v NC bez DPH v tuzemskej mene (desatinné èíslo 2 desatinnými miestami)
    FgDValue := oTxtCut.GetReal(27);  // Hodnota predaja v PC bez DPH pred z¾avou vo vyúètovacej mene (desatinné èíslo 2 desatinnými miestami)
    FgHValue := oTxtCut.GetReal(28);  // Hodnota predaja v PC s DPH pred z¾avou vo vyúètovacej mene (desatinné èíslo 2 desatinnými miestami)
    DscPrc := oTxtCut.GetReal(29);    // Percentuálna hodnota z¾avy (desatinné èíslo 2 desatinnými miestami)
    DscType := oTxtCut.GetText(30);   // Typ z¾avy (text max 1 znak)
    FgAValue := oTxtCut.GetReal(31);  // Hodnota predaja v PC bez DPH po z¾ave vo vyúètovacej mene (desatinné èíslo 2 desatinnými miestami)
    FgBValue := oTxtCut.GetReal(32);  // Hodnota predaja v PC s DPH po z¾ave vo vyúètovacej mene (desatinné èíslo 2 desatinnými miestami)
    FgCourse := oTxtCut.GetReal(33);  // Kurz vyúètovacej meny (desatinné èíslo 5 desatinnými miestami)
    FgDvzName := oTxtCut.GetText(34); // Názov vyúètovacej meny (text max 3 znaky)
  end;
end;

procedure TCssFile.SaveToFile (pDocDate:TDateTime);  // Ulozi zadany udaje do textoveho suboru
var mFileName:string;
begin
  If oFile.Count>0 then begin
    If not DirectoryExists (gPath.StiPath) then ForceDirectories (gPath.StiPath);
    mFileName := gPath.StiPath+'SAL'+StrIntZero(gIni.CrpNum,2)+StrIntZero(gvSys.WriNum,3)+DateToFileName(pDocDate)+'.TXT';
...
procedure TCssFile.LoadFromFile  (pFileName:ShortString);  // Nacita udaje z textoveho suboru
begin
