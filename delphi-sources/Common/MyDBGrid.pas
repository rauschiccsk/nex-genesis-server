...
{ TMyColumn defines internal storage for column attributes.  If IsStored is
...
type
  TMyColumn = class;
  TMyCustomDBGrid = class;

  TMyColumnTitle = class(TPersistent)
  private
    FColumn: TMyColumn;
    FCaption: string;
    FFont: TFont;
  ...
  public
    constructor Create(Column: TMyColumn);
    ...
    property Column: TMyColumn read FColumn;
  published
  ...
  TMyColumn = class(TCollectionItem)
  private
    FField: TField;
    FFieldName: string;
    FColor: TColor;
    FWidth: Integer;
    FTitle: TMyColumnTitle;
    FFont: TFont;
    FImeMode: TImeMode;
    FImeName: TImeName;
    FPickList: TStrings;
    ...
    function  GetParentColumn: TMyColumn;
    ...
    procedure SetTitle(Value: TMyColumnTitle);
    procedure SetWidth(Value: Integer); virtual;
    procedure SetVisible(Value: Boolean);
    function GetExpandable: Boolean;
  protected
    function  CreateTitle: TMyColumnTitle; virtual;
    function  GetGrid: TMyCustomDBGrid;
  ...
  published
    ...
    property  Title: TMyColumnTitle read FTitle write SetTitle;
    property  Width: Integer read GetWidth write SetWidth stored IsWidthStored;
    property  Visible: Boolean read GetVisible write SetVisible;
  end;

  TMyColumnClass = class of TMyColumn;

  TMyDBGridColumns = class(TCollection)
  private
    FGrid: TMyCustomDBGrid;
    function GetColumn(Index: Integer): TMyColumn;
    function InternalAdd: TMyColumn;
    procedure SetColumn(Index: Integer; Value: TMyColumn);
  ...
  public
    constructor Create(Grid: TMyCustomDBGrid; ColumnClass: TMyColumnClass);
    function  Add: TMyColumn;
    ...
    property Grid: TMyCustomDBGrid read FGrid;
    property Items[Index: Integer]: TMyColumn read GetColumn write SetColumn; default;
  end;

  TMyGridDataLink = class(TDataLink)
  private
    FGrid: TMyCustomDBGrid;
    FFieldCount: Integer;
  ...
  public
    constructor Create(AGrid: TMyCustomDBGrid);
    destructor Destroy; override;
  ...
  TMyCustomDBGrid = class(TMyCustomGrid)
  private
    ...
    FSelecting: Boolean;
    FSelRow: Integer;
    FDataLink: TMyGridDataLink;
    FOnColEnter: TNotifyEvent;
    FOnColExit: TNotifyEvent;
    FOnDrawDataCell: TDrawDataCellEvent;
    FOnDrawColumnCell: TDrawColumnCellEvent;
    FEditText: string;
    FColumns: TMyDBGridColumns;
    ...
    function PtInExpandButton(X,Y: Integer; var MasterCol: TMyColumn): Boolean;
    procedure ReadColumns(Reader: TReader);
    procedure RecordChanged(Field: TField);
    procedure SetIme;
    procedure SetColumns(Value: TMyDBGridColumns);
  ...
  protected
    ...
    procedure CellClick(Column: TMyColumn); dynamic;
    procedure ColumnMoved(FromIndex, ToIndex: Longint); override;
    function CalcTitleRect(Col: TMyColumn; ARow: Integer;
      var MasterCol: TMyColumn): TRect;
    function ColumnAtDepth(Col: TMyColumn; ADepth: Integer): TMyColumn;
    procedure ColEnter; dynamic;
    procedure ColExit; dynamic;
    procedure ColWidthsChanged; override;
    function  CreateColumns: TMyDBGridColumns; dynamic;
    function  CreateEditor: TMyInplaceEdit; override;
    ...
    procedure DrawColumnCell(const Rect: TRect; DataCol: Integer;
      Column: TMyColumn; State: TGridDrawState); dynamic;
    ...
    procedure TitleClick(Column: TMyColumn); dynamic;
    ...
    function EndColumnDrag(var Origin, Destination: Integer;
      const MousePt: TPoint): Boolean; override;
    property Columns: TMyDBGridColumns read FColumns write SetColumns;
    property DefaultDrawing: Boolean read FDefaultDrawing write FDefaultDrawing default True;
    property DataLink: TMyGridDataLink read FDataLink;
    ...
    property SelectedRows: TMyBookmarkList read FBookmarks;
  ...
  public
    ...
    procedure DefaultDrawColumnCell(const Rect: TRect; DataCol: Integer;
      Column: TMyColumn; State: TGridDrawState);
    procedure DefaultHandler(var Msg); override;
    function ExecuteAction(Action: TBasicAction): Boolean; override;
    procedure ShowPopupEditor(Column: TMyColumn; X: Integer = Low(Integer);
  ...
  TMyDBGrid = class(TMyCustomDBGrid)
  public
    property Canvas;
...
{ TMyDBGridInplaceEdit }

{ TMyDBGridInplaceEdit adds support for a button on the in-place editor,
...
type
  ...
  TMyDBGridInplaceEdit = class(TMyInplaceEdit)
  private
...
begin
  ...
  TMyDBGridInplaceEdit(Owner).CloseUp((X >= 0) and (Y >= 0) and
...
procedure TMyDBGridInplaceEdit.BoundsChanged;
var
  R: TRect;
begin
  SetRect(R, 2, 2, Width - 2, Height);
  if FEditStyle <> esSimple then
    if not TMyCustomDBGrid(Owner).UseRightToLeftAlignment then
...
procedure TMyDBGridInplaceEdit.CloseUp(Accept: Boolean);
var
  MasterField: TField;
  ListValue: Variant;
begin
  ...
  begin
    ...
    if Accept then
      if FActiveList = FDataList then
        with TMyCustomDBGrid(Grid), Columns[SelectedIndex].Field do
        begin
...
procedure TMyDBGridInplaceEdit.DoDropDownKeys(var Key: Word; Shift: TShiftState);
begin
  case Key of
    VK_UP, VK_DOWN:
...
procedure TMyDBGridInplaceEdit.DropDown;
var
  P: TPoint;
  I,J,Y: Integer;
  Column: TMyColumn;
...
procedure TMyDBGridInplaceEdit.KeyDown(var Key: Word; Shift: TShiftState);
begin
  if (EditStyle = esEllipsis) and (Key = VK_RETURN) and (Shift = [ssCtrl]) then
  begin
    TMyCustomDBGrid(Grid).EditButtonClick;
...
procedure TMyDBGridInplaceEdit.ListMouseUp(Sender: TObject; Button: TMouseButton;
  Shift: TShiftState; X, Y: Integer);
begin
...
procedure TMyDBGridInplaceEdit.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
...
procedure TMyDBGridInplaceEdit.MouseMove(Shift: TShiftState; X, Y: Integer);
var
  ListPos: TPoint;
...
procedure TMyDBGridInplaceEdit.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  WasPressed: Boolean;
begin
...
procedure TMyDBGridInplaceEdit.PaintWindow(DC: HDC);
var
  R: TRect;
  Flags: Integer;
  W, X, Y: Integer;
...
procedure TMyDBGridInplaceEdit.SetEditStyle(Value: TEditStyle);
begin
...
procedure TMyDBGridInplaceEdit.StopTracking;
begin
  if FTracking then
  begin
    TrackButton(-1, -1);
...
procedure TMyDBGridInplaceEdit.TrackButton(X,Y: Integer);
var
  NewState: Boolean;
  R: TRect;
begin
...
procedure TMyDBGridInplaceEdit.UpdateContents;
var
  Column: TMyColumn;
  NewStyle: TEditStyle;
  MasterField: TField;
...
procedure TMyDBGridInplaceEdit.CMCancelMode(var Message: TCMCancelMode);
begin
  if (Message.Sender <> Self) and (Message.Sender <> FActiveList) then
    CloseUp(False);
end;

procedure TMyDBGridInplaceEdit.WMCancelMode(var Message: TMessage);
begin
  StopTracking;
  inherited;
end;

procedure TMyDBGridInplaceEdit.WMKillFocus(var Message: TMessage);
begin
  ...
  begin
    ...
    ImeMode := imDontCare;
    inherited;
    if HWND(Message.WParam) <> TMyCustomDBGrid(Grid).Handle then
      ActivateKeyboardLayout(Screen.DefaultKbLayout, KLF_ACTIVATE);
  end;
  CloseUp(False);
end;

function TMyDBGridInplaceEdit.ButtonRect: TRect;
begin
  if not TMyCustomDBGrid(Owner).UseRightToLeftAlignment then
...
function TMyDBGridInplaceEdit.OverButton(const P: TPoint): Boolean;
begin
  Result := PtInRect(ButtonRect, P);
end;

procedure TMyDBGridInplaceEdit.WMLButtonDblClk(var Message: TWMLButtonDblClk);
begin
  with Message do
...
procedure TMyDBGridInplaceEdit.WMPaint(var Message: TWMPaint);
begin
  PaintHandler(Message);
end;

procedure TMyDBGridInplaceEdit.WMSetCursor(var Message: TWMSetCursor);
var
  P: TPoint;
begin
  GetCursorPos(P);
...
constructor TMyGridDataLink.Create(AGrid: TMyCustomDBGrid);
begin
  inherited Create;
  FGrid := AGrid;
  VisualControl := True;
...
destructor TMyGridDataLink.Destroy;
begin
  ClearMapping;
  inherited Destroy;
end;

function TMyGridDataLink.GetDefaultFields: Boolean;
var
  I: Integer;
begin
  Result := True;
...
function TMyGridDataLink.GetFields(I: Integer): TField;
begin
...
function TMyGridDataLink.AddMapping(const FieldName: string): Boolean;
var
  Field: TField;
  NewSize: Integer;
begin
...
procedure TMyGridDataLink.ActiveChanged;
begin
  FGrid.LinkActive(Active);
  FModified := False;
end;

procedure TMyGridDataLink.ClearMapping;
begin
  FFieldMap := nil;
  FFieldCount := 0;
end;

procedure TMyGridDataLink.Modified;
begin
  FModified := True;
end;

procedure TMyGridDataLink.DataSetChanged;
begin
  FGrid.DataChanged;
  FModified := False;
end;

procedure TMyGridDataLink.DataSetScrolled(Distance: Integer);
begin
  FGrid.Scroll(Distance);
end;

procedure TMyGridDataLink.LayoutChanged;
var
  SaveState: Boolean;
begin
...
procedure TMyGridDataLink.FocusControl(Field: TFieldRef);
begin
...
procedure TMyGridDataLink.EditingChanged;
begin
  FGrid.EditingChanged;
end;

procedure TMyGridDataLink.RecordChanged(Field: TField);
begin
  FGrid.RecordChanged(Field);
  FModified := False;
end;

procedure TMyGridDataLink.UpdateData;
begin
  FInUpdateData := True;
  try
...
function TMyGridDataLink.GetMappedIndex(ColIndex: Integer): Integer;
begin
...
procedure TMyGridDataLink.Reset;
begin
  if FModified then RecordChanged(nil) else Dataset.Cancel;
end;

function TMyGridDataLink.IsAggRow(Value: Integer): Boolean;
begin
  Result := False;
end;

procedure TMyGridDataLink.BuildAggMap;
begin
end;

{ TMyColumnTitle }
constructor TMyColumnTitle.Create(Column: TMyColumn);
begin
  inherited Create;
  FColumn := Column;
  FFont := TFont.Create;
...
destructor TMyColumnTitle.Destroy;
begin
  FFont.Free;
  inherited Destroy;
end;

procedure TMyColumnTitle.Assign(Source: TPersistent);
begin
  if Source is TMyColumnTitle then
  begin
    if cvTitleAlignment in TMyColumnTitle(Source).FColumn.FAssignedValues then
      Alignment := TMyColumnTitle(Source).Alignment;
    if cvTitleColor in TMyColumnTitle(Source).FColumn.FAssignedValues then
      Color := TMyColumnTitle(Source).Color;
    if cvTitleCaption in TMyColumnTitle(Source).FColumn.FAssignedValues then
      Caption := TMyColumnTitle(Source).Caption;
    if cvTitleFont in TMyColumnTitle(Source).FColumn.FAssignedValues then
      Font := TMyColumnTitle(Source).Font;
  end
  else
    inherited Assign(Source);
end;

function TMyColumnTitle.DefaultAlignment: TAlignment;
begin
  Result := taLeftJustify;
end;

function TMyColumnTitle.DefaultColor: TColor;
var
  Grid: TMyCustomDBGrid;
begin
...
function TMyColumnTitle.DefaultFont: TFont;
var
  Grid: TMyCustomDBGrid;
begin
  Grid := FColumn.GetGrid;
...
function TMyColumnTitle.DefaultCaption: string;
var
  Field: TField;
begin
  Field := FColumn.Field;
...
procedure TMyColumnTitle.FontChanged(Sender: TObject);
begin
  Include(FColumn.FAssignedValues, cvTitleFont);
  FColumn.Changed(True);
end;

function TMyColumnTitle.GetAlignment: TAlignment;
begin
...
function TMyColumnTitle.GetColor: TColor;
begin
...
function TMyColumnTitle.GetCaption: string;
begin
...
function TMyColumnTitle.GetFont: TFont;
var
  Save: TNotifyEvent;
  Def: TFont;
begin
...
procedure TMyColumnTitle.RestoreDefaults;
var
...
procedure TMyColumnTitle.SetAlignment(Value: TAlignment);
begin
...
procedure TMyColumnTitle.SetColor(Value: TColor);
begin
...
procedure TMyColumnTitle.SetFont(Value: TFont);
begin
  FFont.Assign(Value);
end;

procedure TMyColumnTitle.SetCaption(const Value: string);
var
  Grid: TMyCustomDBGrid;
begin
...
constructor TMyColumn.Create(Collection: TCollection);
var
  Grid: TMyCustomDBGrid;
begin
  Grid := nil;
  if Assigned(Collection) and (Collection is TMyDBGridColumns) then
    Grid := TMyDBGridColumns(Collection).Grid;
...
procedure TMyColumn.Assign(Source: TPersistent);
begin
  if Source is TMyColumn then
  begin
    if Assigned(Collection) then Collection.BeginUpdate;
    try
      RestoreDefaults;
      FieldName := TMyColumn(Source).FieldName;
      if cvColor in TMyColumn(Source).AssignedValues then
        Color := TMyColumn(Source).Color;
      if cvWidth in TMyColumn(Source).AssignedValues then
        Width := TMyColumn(Source).Width;
      if cvFont in TMyColumn(Source).AssignedValues then
        Font := TMyColumn(Source).Font;
      if cvImeMode in TMyColumn(Source).AssignedValues then
        ImeMode := TMyColumn(Source).ImeMode;
      if cvImeName in TMyColumn(Source).AssignedValues then
        ImeName := TMyColumn(Source).ImeName;
      if cvAlignment in TMyColumn(Source).AssignedValues then
        Alignment := TMyColumn(Source).Alignment;
      if cvReadOnly in TMyColumn(Source).AssignedValues then
        ReadOnly := TMyColumn(Source).ReadOnly;
      Title := TMyColumn(Source).Title;
      DropDownRows := TMyColumn(Source).DropDownRows;
      ButtonStyle := TMyColumn(Source).ButtonStyle;
      PickList := TMyColumn(Source).PickList;
      PopupMenu := TMyColumn(Source).PopupMenu;
      FVisible := TMyColumn(Source).FVisible;
      FExpanded := TMyColumn(Source).FExpanded;
    finally
...
function TMyColumn.CreateTitle: TMyColumnTitle;
begin
  Result := TMyColumnTitle.Create(Self);
end;

function TMyColumn.DefaultAlignment: TAlignment;
begin
  if Assigned(Field) then
...
function TMyColumn.DefaultColor: TColor;
var
  Grid: TMyCustomDBGrid;
begin
  Grid := GetGrid;
...
function TMyColumn.DefaultFont: TFont;
var
  Grid: TMyCustomDBGrid;
begin
  Grid := GetGrid;
...
function TMyColumn.DefaultImeMode: TImeMode;
var
  Grid: TMyCustomDBGrid;
begin
  Grid := GetGrid;
...
function TMyColumn.DefaultImeName: TImeName;
var
  Grid: TMyCustomDBGrid;
begin
  Grid := GetGrid;
...
function TMyColumn.DefaultReadOnly: Boolean;
var
  Grid: TMyCustomDBGrid;
begin
  Grid := GetGrid;
...
function TMyColumn.DefaultWidth: Integer;
var
  W: Integer;
...
procedure TMyColumn.FontChanged;
begin
...
function TMyColumn.GetAlignment: TAlignment;
begin
...
function TMyColumn.GetColor: TColor;
begin
...
function TMyColumn.GetExpanded: Boolean;
begin
  Result := FExpanded and Expandable;
end;

function TMyColumn.GetField: TField;
var
  Grid: TMyCustomDBGrid;
...
function TMyColumn.GetFont: TFont;
var
  Save: TNotifyEvent;
begin
...
function TMyColumn.GetGrid: TMyCustomDBGrid;
begin
  if Assigned(Collection) and (Collection is TMyDBGridColumns) then
    Result := TMyDBGridColumns(Collection).Grid
  else
    Result := nil;
end;

function TMyColumn.GetDisplayName: string;
begin
  Result := FFieldName;
  if Result = '' then Result := inherited GetDisplayName;
end;

function TMyColumn.GetImeMode: TImeMode;
begin
...
function TMyColumn.GetImeName: TImeName;
begin
...
function TMyColumn.GetParentColumn: TMyColumn;
var
  Col: TMyColumn;
  Fld: TField;
  I: Integer;
begin
  ...
  if (Fld <> nil) and (Fld.ParentField <> nil) and (Collection <> nil) then
    ...
    begin
      Col := TMyColumn(Collection.Items[I]);
...
function TMyColumn.GetPickList: TStrings;
begin
  if FPickList = nil then
...
function TMyColumn.GetReadOnly: Boolean;
begin
...
function TMyColumn.GetShowing: Boolean;
var
  Col: TMyColumn;
begin
...
function TMyColumn.GetVisible: Boolean;
var
  Col: TMyColumn;
begin
  Result := FVisible;
...
function TMyColumn.GetWidth: Integer;
begin
  if not Showing then
    Result := -1
...
procedure TMyColumn.RestoreDefaults;
var
  FontAssigned: Boolean;
begin
...
procedure TMyColumn.SetAlignment(Value: TAlignment);
var
  Grid: TMyCustomDBGrid;
begin
  if IsStored then
...
procedure TMyColumn.SetButtonStyle(Value: TColumnButtonStyle);
begin
...
procedure TMyColumn.SetColor(Value: TColor);
begin
...
procedure TMyColumn.SetField(Value: TField);
begin
...
procedure TMyColumn.SetFieldName(const Value: String);
var
  AField: TField;
  Grid: TMyCustomDBGrid;
begin
...
procedure TMyColumn.SetFont(Value: TFont);
begin
  FFont.Assign(Value);
  Include(FAssignedValues, cvFont);
  Changed(False);
...
procedure TMyColumn.SetImeMode(Value: TImeMode);
begin
...
procedure TMyColumn.SetImeName(Value: TImeName);
begin
...
procedure TMyColumn.SetIndex(Value: Integer);
var
  Grid: TMyCustomDBGrid;
  Fld: TField;
  I, OldIndex: Integer;
...
begin
  ...
  begin
    ...
    try
      ...
      while (I < Collection.Count) and (TMyColumn(Collection.Items[I]).ParentColumn = Self) do
        Inc(I);
      Dec(I);
...
procedure TMyColumn.SetPickList(Value: TStrings);
begin
  if Value = nil then
  begin
    FPickList.Free;
...
procedure TMyColumn.SetPopupMenu(Value: TPopupMenu);
begin
  FPopupMenu := Value;
  if Value <> nil then Value.FreeNotification(GetGrid);
end;

procedure TMyColumn.SetReadOnly(Value: Boolean);
var
  Grid: TMyCustomDBGrid;
begin
  Grid := GetGrid;
...
procedure TMyColumn.SetTitle(Value: TMyColumnTitle);
begin
  FTitle.Assign(Value);
end;

procedure TMyColumn.SetWidth(Value: Integer);
var
  Grid: TMyCustomDBGrid;
  TM: TTextMetric;
  DoSetWidth: Boolean;
...
procedure TMyColumn.SetVisible(Value: Boolean);
begin
...
procedure TMyColumn.SetExpanded(Value: Boolean);
const
...
function TMyColumn.Depth: Integer;
var
  Col: TMyColumn;
begin
  Result := 0;
...
function TMyColumn.GetExpandable: Boolean;
var
  Fld: TField;
begin
  Fld := Field;
...
constructor TMyDBGridColumns.Create(Grid: TMyCustomDBGrid; ColumnClass: TMyColumnClass);
begin
  inherited Create(ColumnClass);
  FGrid := Grid;
end;

function TMyDBGridColumns.Add: TMyColumn;
begin
  Result := TMyColumn(inherited Add);
end;

function TMyDBGridColumns.GetColumn(Index: Integer): TMyColumn;
begin
  Result := TMyColumn(inherited Items[Index]);
end;

function TMyDBGridColumns.GetOwner: TPersistent;
begin
  Result := FGrid;
end;

procedure TMyDBGridColumns.LoadFromFile(const Filename: string);
var
  S: TFileStream;
begin
...
type
  TMyColumnsWrapper = class(TComponent)
  private
    FColumns: TMyDBGridColumns;
  published
    property Columns: TMyDBGridColumns read FColumns write FColumns;
  end;

procedure TMyDBGridColumns.LoadFromStream(S: TStream);
var
  Wrapper: TMyColumnsWrapper;
begin
  Wrapper := TMyColumnsWrapper.Create(nil);
  try
...
procedure TMyDBGridColumns.RestoreDefaults;
var
  I: Integer;
begin
  BeginUpdate;
...
procedure TMyDBGridColumns.RebuildColumns;
...
procedure TMyDBGridColumns.SaveToFile(const Filename: string);
var
  S: TStream;
begin
...
procedure TMyDBGridColumns.SaveToStream(S: TStream);
var
  Wrapper: TMyColumnsWrapper;
begin
  Wrapper := TMyColumnsWrapper.Create(nil);
...
procedure TMyDBGridColumns.SetColumn(Index: Integer; Value: TMyColumn);
begin
  Items[Index].Assign(Value);
end;

procedure TMyDBGridColumns.SetState(NewState: TDBGridColumnsState);
begin
...
procedure TMyDBGridColumns.Update(Item: TCollectionItem);
var
  Raw: Integer;
begin
  ...
  begin
    ...
    FGrid.ColWidths[Raw] := TMyColumn(Item).Width;
  end;
end;

function TMyDBGridColumns.InternalAdd: TMyColumn;
begin
  Result := Add;
  Result.IsStored := False;
end;

function TMyDBGridColumns.GetState: TDBGridColumnsState;
begin
...
constructor TMyBookmarkList.Create(AGrid: TMyCustomDBGrid);
begin
  inherited Create;
...
procedure TMyBookmarkList.Clear;
begin
...
function TMyBookmarkList.Find(const Item: TBookmarkStr; var Index: Integer): Boolean;
var
  L, H, I, C: Integer;
begin
...
function TMyBookmarkList.GetCount: Integer;
begin
  Result := FList.Count;
end;

function TMyBookmarkList.GetItem(Index: Integer): TBookmarkStr;
begin
  Result := FList[Index];
end;

function TMyBookmarkList.IndexOf(const Item: TBookmarkStr): Integer;
begin
  if not Find(Item, Result) then
    Result := -1;
end;

procedure TMyBookmarkList.LinkActive(Value: Boolean);
begin
  Clear;
  FLinkActive := Value;
end;

procedure TMyBookmarkList.Delete;
var
  I: Integer;
begin
  with FGrid.Datalink.Datasource.Dataset do
...
function TMyBookmarkList.Refresh: Boolean;
var
  I: Integer;
begin
  Result := False;
...
procedure TMyBookmarkList.SetCurrentRowSelected(Value: Boolean);
var
  Index: Integer;
...
procedure TMyBookmarkList.StringsChanged(Sender: TObject);
begin
  FCache := '';
  FCacheIndex := -1;
end;
...
constructor TMyCustomDBGrid.Create(AOwner: TComponent);
var
  Bmp: TBitmap;
begin
  ...
  FDataLink := TMyGridDataLink.Create(Self);
  Color := clWindow;
  ParentColor := False;
  ...
  FBookmarks := TMyBookmarkList.Create(Self);
  HideEditor;
end;

destructor TMyCustomDBGrid.Destroy;
begin
  FColumns.Free;
  FColumns := nil;
...
function TMyCustomDBGrid.AcquireFocus: Boolean;
begin
  Result := True;
...
function TMyCustomDBGrid.RawToDataColumn(ACol: Integer): Integer;
begin
  Result := ACol - FIndicatorOffset;
end;

function TMyCustomDBGrid.DataToRawColumn(ACol: Integer): Integer;
begin
  Result := ACol + FIndicatorOffset;
end;

function TMyCustomDBGrid.AcquireLayoutLock: Boolean;
begin
  Result := (FUpdateLock = 0) and (FLayoutLock = 0);
  if Result then BeginLayout;
end;

procedure TMyCustomDBGrid.BeginLayout;
begin
  BeginUpdate;
...
procedure TMyCustomDBGrid.BeginUpdate;
begin
  Inc(FUpdateLock);
end;

procedure TMyCustomDBGrid.CancelLayout;
begin
...
function TMyCustomDBGrid.CanEditAcceptKey(Key: Char): Boolean;
begin
  with Columns[SelectedIndex] do
    Result := FDatalink.Active and Assigned(Field) and Field.IsValidChar(Key);
end;

function TMyCustomDBGrid.CanEditModify: Boolean;
begin
  Result := False;
...
function TMyCustomDBGrid.CanEditShow: Boolean;
begin
  Result := (LayoutLock = 0) and inherited CanEditShow;
end;

procedure TMyCustomDBGrid.CellClick(Column: TMyColumn);
begin
  if Assigned(FOnCellClick) then FOnCellClick(TColumn (Column));
end;

procedure TMyCustomDBGrid.ColEnter;
begin
  UpdateIme;
  if Assigned(FOnColEnter) then FOnColEnter(Self);
end;

procedure TMyCustomDBGrid.ColExit;
begin
  if Assigned(FOnColExit) then FOnColExit(Self);
end;

procedure TMyCustomDBGrid.ColumnMoved(FromIndex, ToIndex: Longint);
begin
...
procedure TMyCustomDBGrid.ColWidthsChanged;
var
  I: Integer;
begin
...
function TMyCustomDBGrid.CreateColumns: TMyDBGridColumns;
begin
  Result := TMyDBGridColumns.Create(Self, TMyColumn);
end;

function TMyCustomDBGrid.CreateEditor: TMyInplaceEdit;
begin
  Result := TMyDBGridInplaceEdit.Create(Self);
end;

procedure TMyCustomDBGrid.CreateWnd;
begin
...
procedure TMyCustomDBGrid.DataChanged;
begin
...
procedure TMyCustomDBGrid.DefaultHandler(var Msg);
var
  P: TPopupMenu;
  Cell: TGridCoord;
begin
...
procedure TMyCustomDBGrid.DeferLayout;
var
  M: TMsg;
begin
...
procedure TMyCustomDBGrid.DefineFieldMap;
var
  I: Integer;
begin
...
function TMyCustomDBGrid.UseRightToLeftAlignmentForField(const AField: TField;
  Alignment: TAlignment): Boolean;
begin
  Result := False;
...
procedure TMyCustomDBGrid.DefaultDrawDataCell(const Rect: TRect; Field: TField;
...
procedure TMyCustomDBGrid.DefaultDrawColumnCell(const Rect: TRect;
  DataCol: Integer; Column: TMyColumn; State: TGridDrawState);
var
  Value: string;
begin
  Value := '';
...
procedure TMyCustomDBGrid.ReadColumns(Reader: TReader);
begin
  Columns.Clear;
  Reader.ReadValue;
...
procedure TMyCustomDBGrid.WriteColumns(Writer: TWriter);
begin
...
procedure TMyCustomDBGrid.DefineProperties(Filer: TFiler);
var
  StoreIt: Boolean;
...
begin
  ...
  if Filer.Ancestor = nil then
    ...
    if vState <> TMyCustomDBGrid(Filer.Ancestor).Columns.State then
      StoreIt := True
    else
      StoreIt := (vState = csCustomized) and
        (not CollectionsEqual(Columns, TMyCustomDBGrid(Filer.Ancestor).Columns));

  Filer.DefineProperty('Columns', ReadColumns, WriteColumns, StoreIt);
end;

function TMyCustomDBGrid.ColumnAtDepth(Col: TMyColumn; ADepth: Integer): TMyColumn;
begin
  Result := Col;
...
function TMyCustomDBGrid.CalcTitleRect(Col: TMyColumn; ARow: Integer;
  var MasterCol: TMyColumn): TRect;
var
  I,J: Integer;
  InBiDiMode: Boolean;
...
procedure TMyCustomDBGrid.DrawCell(ACol, ARow: Longint; ARect: TRect; AState: TGridDrawState);
var
  FrameOffs: Byte;
  ...
  procedure DrawTitleCell(ACol, ARow: Integer; Column: TMyColumn; var AState: TGridDrawState);
  const
...
procedure TMyCustomDBGrid.DrawDataCell(const Rect: TRect; Field: TField;
  State: TGridDrawState);
begin
  if Assigned(FOnDrawDataCell) then FOnDrawDataCell(Self, Rect, Field, State);
end;

procedure TMyCustomDBGrid.DrawColumnCell(const Rect: TRect; DataCol: Integer;
  Column: TMyColumn; State: TGridDrawState);
begin
  if Assigned(OnDrawColumnCell) then
    OnDrawColumnCell(Self, Rect, DataCol, TColumn (Column), State);
end;

procedure TMyCustomDBGrid.EditButtonClick;
begin
...
procedure TMyCustomDBGrid.EditingChanged;
begin
  if dgIndicator in Options then InvalidateCell(0, FSelRow);
end;

procedure TMyCustomDBGrid.EndLayout;
begin
  if FLayoutLock > 0 then
  begin
    try
...
procedure TMyCustomDBGrid.EndUpdate;
begin
  if FUpdateLock > 0 then
    Dec(FUpdateLock);
end;

function TMyCustomDBGrid.GetColField(DataCol: Integer): TField;
begin
  Result := nil;
...
function TMyCustomDBGrid.GetDataSource: TDataSource;
begin
  Result := FDataLink.DataSource;
end;

function TMyCustomDBGrid.GetEditLimit: Integer;
begin
  Result := 0;
...
function TMyCustomDBGrid.GetEditMask(ACol, ARow: Longint): string;
begin
  Result := '';
...
function TMyCustomDBGrid.GetEditText(ACol, ARow: Longint): string;
begin
  Result := '';
  if FDatalink.Active then
  with Columns[RawToDataColumn(ACol)] do
...
function TMyCustomDBGrid.GetFieldCount: Integer;
begin
  Result := FDatalink.FieldCount;
end;

function TMyCustomDBGrid.GetFields(FieldIndex: Integer): TField;
begin
  Result := FDatalink.Fields[FieldIndex];
end;

function TMyCustomDBGrid.GetFieldValue(ACol: Integer): string;
var
  Field: TField;
begin
  Result := '';
...
function TMyCustomDBGrid.GetSelectedField: TField;
var
  Index: Integer;
begin
  Index := SelectedIndex;
...
function TMyCustomDBGrid.GetSelectedIndex: Integer;
begin
  Result := RawToDataColumn(Col);
end;

function TMyCustomDBGrid.HighlightCell(DataCol, DataRow: Integer;
...
procedure TMyCustomDBGrid.KeyDown(var Key: Word; Shift: TShiftState);
var
...
procedure TMyCustomDBGrid.KeyPress(var Key: Char);
begin
  FIsESCKey := False;
...
procedure TMyCustomDBGrid.InternalLayout;
...
procedure TMyCustomDBGrid.LayoutChanged;
begin
  if AcquireLayoutLock then
    EndLayout;
end;

procedure TMyCustomDBGrid.LinkActive(Value: Boolean);
var
  Comp: TComponent;
  I: Integer;
begin
  ...
  try
    ...
    begin
      ...
      if (Comp is TMyCustomDBGrid)
        and (TMyCustomDBGrid(Comp).DragKind = dkDock) then
        Comp.Free;
    end;
    UpdateScrollBar;
...
procedure TMyCustomDBGrid.Loaded;
begin
  inherited Loaded;
...
function TMyCustomDBGrid.PtInExpandButton(X,Y: Integer; var MasterCol: TMyColumn): Boolean;
var
  Cell: TGridCoord;
  R: TRect;
begin
...
procedure TMyCustomDBGrid.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  Cell: TGridCoord;
...
procedure TMyCustomDBGrid.MouseUp(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
var
  Cell: TGridCoord;
...
procedure TMyCustomDBGrid.MoveCol(RawCol, Direction: Integer);
var
  OldCol: Integer;
begin
  FDatalink.UpdateData;
...
procedure TMyCustomDBGrid.Notification(AComponent: TComponent;
...
procedure TMyCustomDBGrid.RecordChanged(Field: TField);
var
  I: Integer;
  CField: TField;
begin
...
procedure TMyCustomDBGrid.Scroll(Distance: Integer);
var
...
procedure TMyCustomDBGrid.SetColumns(Value: TMyDBGridColumns);
begin
...
procedure TMyCustomDBGrid.SetColumnAttributes;
var
  I: Integer;
begin
  for I := 0 to FColumns.Count-1 do
...
procedure TMyCustomDBGrid.SetDataSource(Value: TDataSource);
begin
...
procedure TMyCustomDBGrid.SetEditText(ACol, ARow: Longint; const Value: string);
begin
  FEditText := Value;
end;

procedure TMyCustomDBGrid.SetOptions(Value: TDBGridOptions);
const
...
procedure TMyCustomDBGrid.SetSelectedField(Value: TField);
var
  I: Integer;
begin
...
procedure TMyCustomDBGrid.SetSelectedIndex(Value: Integer);
begin
  MoveCol(DataToRawColumn(Value), 0);
end;

procedure TMyCustomDBGrid.SetTitleFont(Value: TFont);
begin
  FTitleFont.Assign(Value);
  if dgTitles in Options then LayoutChanged;
end;

function TMyCustomDBGrid.StoreColumns: Boolean;
begin
  Result := Columns.State = csCustomized;
end;

procedure TMyCustomDBGrid.TimedScroll(Direction: TGridScrollDirection);
begin
  if FDatalink.Active then
  begin
    with FDatalink do
...
procedure TMyCustomDBGrid.TitleClick(Column: TMyColumn);
begin
  if Assigned(FOnTitleClick) then FOnTitleClick(TColumn (Column));
end;

procedure TMyCustomDBGrid.TitleFontChanged(Sender: TObject);
begin
...
procedure TMyCustomDBGrid.UpdateActive;
var
  NewRow: Integer;
  Field: TField;
begin
...
procedure TMyCustomDBGrid.UpdateData;
var
  Field: TField;
begin
  Field := SelectedField;
...
procedure TMyCustomDBGrid.UpdateRowCount;
var
  OldRowCount: Integer;
begin
...
procedure TMyCustomDBGrid.UpdateScrollBar;
var
...
function TMyCustomDBGrid.ValidFieldIndex(FieldIndex: Integer): Boolean;
begin
  Result := DataLink.GetMappedIndex(FieldIndex) >= 0;
end;

procedure TMyCustomDBGrid.CMParentFontChanged(var Message: TMessage);
begin
  inherited;
  if ParentFont then
  begin
...
procedure TMyCustomDBGrid.CMBiDiModeChanged(var Message: TMessage);
var
  Loop: Integer;
begin
  inherited;
  for Loop := 0 to ComponentCount - 1 do
    if Components[Loop] is TMyCustomDBGrid then
      with Components[Loop] as TMyCustomDBGrid do
        { Changing the window, echos down to the subgrid }
        if Parent <> nil then
          Parent.BiDiMode := Self.BiDiMode;
end;

procedure TMyCustomDBGrid.CMExit(var Message: TMessage);
begin
  try
    if FDatalink.Active then
      with FDatalink.Dataset do
...
procedure TMyCustomDBGrid.CMFontChanged(var Message: TMessage);
var
  I: Integer;
begin
  inherited;
...
procedure TMyCustomDBGrid.CMDeferLayout(var Message);
begin
  if AcquireLayoutLock then
    EndLayout
  else
...
procedure TMyCustomDBGrid.CMDesignHitTest(var Msg: TCMDesignHitTest);
var
  MasterCol: TMyColumn;
begin
  inherited;
...
procedure TMyCustomDBGrid.WMSetCursor(var Msg: TWMSetCursor);
begin
...
procedure TMyCustomDBGrid.WMSize(var Message: TWMSize);
begin
  inherited;
...
procedure TMyCustomDBGrid.WMVScroll(var Message: TWMVScroll);
var
  SI: TScrollInfo;
begin
  if not AcquireFocus then Exit;
...
procedure TMyCustomDBGrid.SetIme;
var
  Column: TMyColumn;
begin
  ...
  begin
    TMyDBGridInplaceEdit(Self).ImeName := ImeName;
    TMyDBGridInplaceEdit(Self).ImeMode := ImeMode;
  end;
end;

procedure TMyCustomDBGrid.UpdateIme;
begin
  if not SysLocale.FarEast then Exit;
  SetIme;
  SetImeName(ImeName);
...
procedure TMyCustomDBGrid.WMIMEStartComp(var Message: TMessage);
begin
  inherited;
  ShowEditor;
end;

procedure TMyCustomDBGrid.WMSetFocus(var Message: TWMSetFocus);
begin
...
procedure TMyCustomDBGrid.WMKillFocus(var Message: TMessage);
begin
  if not SysLocale.FarEast then inherited
  else
  begin
...
function TMyCustomDBGrid.ExecuteAction(Action: TBasicAction): Boolean;
begin
  Result := (DataLink <> nil) and DataLink.ExecuteAction(Action);
end;

function TMyCustomDBGrid.UpdateAction(Action: TBasicAction): Boolean;
begin
  Result := (DataLink <> nil) and DataLink.UpdateAction(Action);
end;

procedure TMyCustomDBGrid.ShowPopupEditor(Column: TMyColumn; X, Y: Integer);
var
  SubGrid: TMyCustomDBGrid;
  DS: TDataSource;
  I: Integer;
  FloatRect: TRect;
  Cmp: TControl;
begin
  ...
  for I := 0 to ComponentCount-1 do
    if Components[I] is TMyCustomDBGrid then
    begin
      SubGrid := TMyCustomDBGrid(Components[I]);
  ...
  SubGrid := TMyCustomDBGrid(TComponentClass(Self.ClassType).Create(Self));
  try
...
//    SubGrid.ManualDock(nil,nil,alClient);
    ...
    Cmp := Self;
    while (Cmp <> nil) and (TMyCustomDBGrid(Cmp).PopupMenu = nil) do
      Cmp := Cmp.Parent;
    if Cmp <> nil then
      SubGrid.PopupMenu := TMyCustomDBGrid(Cmp).PopupMenu;
...
procedure TMyCustomDBGrid.CalcSizingState(X, Y: Integer;
...
function TMyCustomDBGrid.CheckColumnDrag(var Origin, Destination: Integer;
...
function TMyCustomDBGrid.BeginColumnDrag(var Origin, Destination: Integer;
  const MousePt: TPoint): Boolean;
var
  I, ARow: Integer;
begin
...
function TMyCustomDBGrid.EndColumnDrag(var Origin, Destination: Integer;
  const MousePt: TPoint): Boolean;
begin
...
procedure TMyCustomDBGrid.InvalidateTitles;
var
  R: TRect;
  DrawInfo: TGridDrawInfo;
begin
...
procedure TMyCustomDBGrid.TopLeftChanged;
begin
  InvalidateTitles; 
