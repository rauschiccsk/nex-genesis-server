...
{$F+}
(*
Na zatku kad zprvy v ppad odchoz objednvky mus bt systmov zznam - viz. struktura systmovho zznamu

Cislo	ORDERS
poloky                                       Typ        Delka  Pozice   Zarovn   Povinn  Poznamka nebo hodnota poloky
HDR	Hlavika
 1      uvozen "HDR"                         Char       3      1-3      zleva    P       "HDR"
 2      Cislo objednavky                     Char       15     4-18     zleva    P
 3      Datum vystavenia objednavky          Date       8      19-26    zleva    P       format YYYYMMDD
 4      EAN kupujucho - objednatele          Char       17     27-43    zleva    P       EAN kod 13 mst
 5      EAN miesta dodania                   Char       17     44-60    zleva    N       EAN kod 13 mst
 6      EAN fakturacneho msieta              Char       17     61-77    zleva    N       EAN kod 13 mst
 7      EAN dodavatela                       Char       17     78-94    zleva    P       EAN kod 13 mst
 8      EAN distribucneho skladu dodavatele  Char       17     95-111   zleva    N       EAN kod 13 mst
 9      Pozadovany datum dodnia              Date/Time  8      112-119  zleva    N       format YYYYMMDD
10      Pozadovany cas dodnia                Date/Time  4      120-123  zleva    N       format HHMM
11      Volne text - kontaktn osoba,         Char       70     124-193  zleva    N
        telefon, fax atd.
*)
...
type
  TOcdInh = class
    ...
    public
      ...
      oExtNum:Str15;   // Cislo objednavky - externe
      oDocDate:TDateTime; // Datum vystavenia objednavky
      oEanCus:Str17; // EAN kupujuceho
      oEanDlv:Str17; // EAN miesta dodania
      oEanInv:Str17; // EAN fakturacneho miesta
      oEanSup:Str17; // EAN dodavatela
      oEanStk:Str17; // EAN distribucneho skladu dodavatela
      oDlvDate:TDateTime; // Pozadovany datum dodania - termin dodavky
      oDlvTime:TDateTime; // Pozadovany cas dodania - termin dodavky
      oNotice:Str70;  // Poznamka
    ...
    published
      ...
      property DocNum:Str12
      property ExtNum:Str12
      property DocDate:TDateTime
  end;
...
constructor TOcdInh.Create;
begin
end;

destructor TOcdInh.Destroy;
begin
end;

procedure TOcdInh.SaveToFile (pDocNum:Str12);  // Ulozi zadany doklad do textoveho suboru
...
begin
  ...
  If dmSTK.btOCI.FieldByName ('DocNum').AsString=dmSTK.btOCH.FieldByName ('DocNum').AsString then begin
    Repeat
      ...
      (*
      DlrCode    word         ;Kod obchodoeho zastupcu
...
procedure TOcdInh.LoadFromFile (pDocNum:Str12);  // Nacita doklad z textoveho suboru
...
procedure TOcdInh.LoadTxtToOch; // Nacita udaje z prenosoveho suboru a vytvori alebo opravy hlavicku ZK
begin
  ...
  If oTxtDoc.ReadInteger ('FtpVer')=0 then begin  // Povodny format
    ...
    dmSTK.btOCH.FieldByName('RspUser').AsString := oTxtDoc.ReadString ('RspUser');
    ...
    dmSTK.btOCH.FieldByName('VatPrc1').AsFloat := oTxtDoc.ReadInteger ('VatPrc1');
    dmSTK.btOCH.FieldByName('VatPrc2').AsFloat := oTxtDoc.ReadInteger ('VatPrc2');
    dmSTK.btOCH.FieldByName('VatPrc3').AsFloat := oTxtDoc.ReadInteger ('VatPrc3');
  ...
  //  dmSTK.btOCH.FieldByName('ModTime').AsDateTime := oTxtDoc.ReadTime ('ModTime');
  end
  else begin  // FtpVer=2 - nova verzia od v10.02
    dmSTK.btOCH.FieldByName('SerNum').AsInteger := oTxtDoc.ReadInteger ('SerNum');
    dmSTK.btOCH.FieldByName('Year').AsString := oTxtDoc.ReadString ('Year');
    dmSTK.btOCH.FieldByName('DocNum').AsString := oTxtDoc.ReadString ('DocNum');
    dmSTK.btOCH.FieldByName('ExtNum').AsString := oTxtDoc.ReadString ('ExtNum');
...
procedure TOcdInh.LoadTxtToTmp; // Nacita poloky z prenosoveho suboru do docasneh databaze ptTCI
begin
...
procedure TOcdInh.LoadTxtToOcn; // Nacita poloky z prenosoveho suboru do docasneh databaze ptTCI
begin
end;

procedure TOcdInh.DeleteOci; // Vymaze polozky doacieho listu, ktore nie su v docasnej databaze
begin
...
procedure TOcdInh.AddNewItmToOci; // Prida nove polozky do ZK
begin
