...
type
  TXPContainer = (xccForm, xccFrame, xccToolbar, xccCoolbar, xccPanel,
                  xccScrollBox, xccGroupBox, xccTabSheet, xccPageScroller);
  TXPContainers = set of TXPContainer;

  TXPControl = (xcMainMenu, xcPopupMenu, xcToolbar, xcListBox,
                xcMemo, xcRichEdit,
                xcSpeedButton, xcUpDown, xcPanel,
                xcGroupBox, xcTreeView, xcListView, xcProgressBar);
  ...
  private
    ...
    FIconWidth: integer;
    FDrawSelect: boolean;
    FUseWindowsSystemColors: boolean;
...
//   FTransparentColor: TColor;// +jt
   ...
   // Do not allow the component to be used for subclassing
    ...
    procedure SetUseWindowsSystemColors(const Value: boolean);
...
//   property TransparentColor: TColor read FTransparentColor write FTransparentColor;// +jt
  published
    ...
    property UseWindowsSystemColors: boolean read FUseWindowsSystemColors write SetUseWindowsSystemColors;
    ...
    property XPContainers: TXPContainers read FXPContainers write SetXPContainers
      default [xccForm, xccFrame, xccToolbar, xccCoolbar, xccPanel,
                  xccScrollBox, xccGroupBox, xccTabSheet, xccPageScroller];
    property XPControls :TXPControls read FXPControls write SetXPControls
      default [xcMainMenu, xcPopupMenu, xcToolbar, xcListBox,
               xcMemo, xcRichEdit, 
               xcSpeedButton, xcUpDown, xcPanel,
               xcGroupBox, xcTreeView, xcListView, xcProgressBar];
               {xcStringGrid, xcDrawGrid, xcDBGrid];}
...
procedure Register;
begin
  RegisterComponents('xpComp', [TXPMenu]);
end;
...
{$ENDIF}
  ...
  FGradient := TRUE;
  FUseWindowsSystemColors := FALSE;
  FColor := clIccPanelBG;
  FIconBackColor := clIccPanelBG;
  FSelectColor := clHighlight;
  FSelectBorderColor := clHighlight;
  FMenuBarColor := clIccPanelBG;
  FDisabledColor := clInactiveCaption;
  FSeparatorColor := clIccPanelBG;
  ...
  FIconWidth := 24;
  FDrawSelect := true;
  XPContainers := [xccForm, xccFrame, xccToolbar, xccCoolbar, xccPanel,
                  xccScrollBox, xccGroupBox, xccTabSheet, xccPageScroller];
  XPControls := [xcMainMenu, xcPopupMenu, xcToolbar, xcListBox,
                xcMemo, xcRichEdit, 
                xcSpeedButton, xcUpDown, xcPanel,
                xcGroupBox, xcTreeView, xcListView, xcProgressBar];
            {xcStringGrid, xcDrawGrid, xcDBGrid];}
...
begin
  ...
  if not (csDesigning in ComponentState) then
    if ((Comp is TCustomMemo) and (xcMemo in FXPControls)) or
       ...
       ((Comp is TProgressBar) and (xcProgressBar in FXPControls)) 
       then
...
begin
  ...
  if (Is16Bit) and (not UseWindowsSystemColors) then
...
//-----
  ...
  begin
    ...
    FMenuBorderColor := GetShadeColor(ACanvas, clIccPanelBG, 90);
    FMenuShadowColor := GetShadeColor(ACanvas, clIccPanelBG, 76);
  end
  else
  begin
    FFSelectColor := FSelectColor;
  ...
  if FUseWindowsSystemColors then
  begin
//    GetSystemMenuFont(FFont);
    ...
    begin
      FFColor := NewColor(ACanvas, clIccPanelBG, 86);
      FFIconBackColor := NewColor(ACanvas, clIccPanelBG, 16);
      FFSelectColor := NewColor(ACanvas, clHighlight, 68);
      FFSelectBorderColor := clHighlight;
      FFMenuBarColor := clIccPanelBG;
...
begin
  ...
  begin
    ...
    begin
      if (Is16Bit) and (not UseWindowsSystemColors) and (Sender is TToolButton) then
...
procedure TXPMenu.SetUseWindowsSystemColors(const Value: boolean);
begin
  FUseWindowsSystemColors := Value;
  Windows.DrawMenuBar(FForm.Handle);
end;
...
//----------
    ...
    begin
      ...
      begin
        SelectColor := NewColor(C, clIccPanelBG, xpMenu.FDimParentColorSelect);
        if FFlat then
          SelectColor := xpMenu.FFSelectColor ;
      end;
      BorderColor := NewColor(C, XPMenu.FFSelectBorderColor,60);
  ...
  begin
    ...
    begin
      ...
      if Control.Tag = 1000 then
        ...
        SelectColor := NewColor(C, XPMenu.FFSelectColor, xpMenu.FDimParentColor);//clIccPanelBG;
